...
# -----------------------------------------------------------
# 🔥 RENDER FIX: Event Loop Fix & Web Server
# -----------------------------------------------------------
try:
...
# গ্রুপ ট্র্যাকিং (ব্রডকাস্টের জন্য জরুরি)
...
**📝 Filters & Welcome:**
/save <word> - Save filter
/setwelcome - Set welcome message
/resetwelcome - Reset welcome

**📢 Broadcast:**
/broadcast - Send message to all groups (Owner Only)
"""
    await m.reply(text)

# -----------------------------------------------------------
# ✅ নতুন: BROADCAST কমান্ড (শুধু ওনারের জন্য)
# -----------------------------------------------------------
@app.on_message(filters.command("broadcast") & filters.user(OWNER_ID))
async def broadcast_msg(c, m):
    if not m.reply_to_message:
        return await m.reply("❌ **Error:** দয়া করে কোনো মেসেজে রিপ্লাই দিয়ে কমান্ডটি দিন।")

    msg = await m.reply("⏳ **Broadcasting Started...**")
    sent = 0
    failed = 0
    
    # মেমোরিতে থাকা সব গ্রুপে লুপ চালাবে
    for chat_id in served_chats:
        try:
            # মেসেজটি কপি করে পাঠাবে
            await m.reply_to_message.copy(chat_id)
            sent += 1
            await asyncio.sleep(0.1) # ফ্লাডওয়েট এড়াতে ছোট ডিলে
        except Exception:
            failed += 1
...
async def auto_moderation(c, m):
    ...
    if m.forward_date or m.forward_from or m.forward_from_chat:
        violation = True
        reason = "মেসেজ ফরওয়ার্ড করা নিষিদ্ধ!"
    elif re.search(link_pattern, msg_text):
        violation = True
        reason = "লিংক বা ইউজারনেম শেয়ার করা নিষিদ্ধ!"
    ...
    if violation:
        ...
        if current_warn >= 3:
            ...
            msg = await m.reply(f"⚠️ {m.from_user.mention}, {reason}\nসতর্কবার্তা: {current_warn}/3")
...
async def admin_tools(c, m):
    if not await is_admin(m): return await m.reply("❌ **Access Denied!**")
    cmd = m.command[0]
    chat_id = m.chat.id
    ...
    try:
        if cmd == "ban":
            ...
            await c.restrict_chat_member(chat_id, target.id, ChatPermissions(can_send_messages=True, can_send_media_messages=True, can_send_other_messages=True, can_invite_users=True))
            if chat_id in warns_db and target.id in warns_db[chat_id]: warns_db[chat_id][target.id] = 0
            await m.reply(f"🔊 **Unmuted:** {target.mention} এবং ওয়ার্নিং রিসেট করা হয়েছে।")
        elif cmd == "kick":
            await c.ban_chat_member(chat_id, target.id)
            await c.unban_chat_member(chat_id, target.id)
...
async def main():
    await start_server()
    await app.start()
    print("✅ Bot Started Successfully with Broadcast & Help!")
    await idle()
    await app.stop()
